...
public class FlatFilesDelimitedWriteConfiguration {
    ...
    @Bean
    public Job flatFilesDelimitedWriteJob() throws Exception {
        return jobBuilderFactory.get("flatFilesDelimitedWriteJob")
                .incrementer(new RunIdIncrementer())
                .start(flatFilesDelimitedWriteStep1())
                .build();
    }

    @Bean
    public Step flatFilesDelimitedWriteStep1() throws Exception {
        return stepBuilderFactory.get("flatFilesDelimitedWriteStep1")
                .<String,String>chunk(1)
                .reader(flatFilesDelimitedWriteCustomItemReader())
                .writer(flatFilesDelimitedWriteCustomItemWriter())
                .build();
    }

    @Bean
    public ListItemReader flatFilesDelimitedWriteCustomItemReader() {
    ...
    @Bean
    public FlatFileItemWriter<Customer> flatFilesDelimitedWriteCustomItemWriter() throws Exception {
        /*
           FlatFileItemWriter > doWriter()
           - items : Customer2 객체의 리스트 (3개일때 총 3줄의 String)
    ...
    /*
    @Bean
    public FlatFileItemWriter<Customer2> customItemWriter() throws Exception {
        BeanWrapperFieldExtractor<Customer2> fieldExtractor = new BeanWrapperFieldExtractor<>();
        fieldExtractor.setNames(new String[] {"id","name","age"});
        fieldExtractor.afterPropertiesSet();

        DelimitedLineAggregator<Customer2> lineAggregator = new DelimitedLineAggregator<>();
